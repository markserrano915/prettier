...
function printFunctionParams(path, print, options, expandArg, printTypeParams) {
  ...
  const isParametersShouldBeHugged = shouldHugArguments(fun);
  const isParametersShouldBeExpended =
  ...
  if (fun[paramsField]) {
    ...
    printed = path.map((childPath, index) => {
      ...
      if (index === lastArgIndex) {
        ...
        isParametersShouldBeHugged ||
        isParametersShouldBeExpended
      ) {
        parts.push(", ");
  ...
  //   })                    ) => {
  //                         })
  if (isParametersShouldBeExpended) {
    return group(
      concat([
